...
class Contrast_ReLU_activate(nn.Module):
    ...
    def forward(self, dom_res, dom_label):
        ...
        w = (self.dom_func_weight * self.weight_scale)
        b = (self.dom_func_bias + self.add_bias)
...
class Discriminator_Weights_Adjust(nn.Module):
    ...
    def __init__(self):
        ...
        self.k_var = torch.FloatTensor([-0.9]).cuda()

        self.l1_rev = torch.ones(1).cuda()
        self.l2_rev = torch.ones(1).cuda()
        self.l3_rev = torch.ones(1).cuda()
...
class ICAN(nn.Module):
    ...
    def forward(self, cond, x1, x2=None, l=None, dom_label=None, 
        ...
        if (cond == 'source_train' or cond == 'pretrain' or cond == 'source_pseudo_train'):
            ...
            return class_pred, dom_pred.squeeze(1), dom_pred_l1.squeeze(1), \
                               dom_pred_l2.squeeze(1), dom_pred_l3.squeeze(1), \
            ...
            disc_w, weight, bias = self.disc_activate(dom_pred, dom_label)
